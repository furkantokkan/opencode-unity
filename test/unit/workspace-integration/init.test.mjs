...
import { createCommandHarness, createRunner } from '../commands/helpers.mjs';
...
describe('workspace init integration', () => {
  ...
  it('builds a read-only launch for an initialized standalone service', async (t) => {
    ...
    await fs.writeFile(binary, 'mock binary; execution is injected', 'utf8');
    const runner = createRunner({ '--version': { stdout: `${loadCompat().opencode.tested}\n` } });
    const result = await harness.run('start', { options: { printEnv: true }, env: { [BINARY_ENV_NAME]: binary }, deps: { run: runner.run } });
    assert.equal(result.exitCode, 0, result.message);
    assert.deepEqual(await snapshotTree(harness.projectRoot), before);
  });
